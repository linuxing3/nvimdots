...
local function get_jdtls_config_dir()
    return util.path.join(env.HOME, 'lib/lsp/jdtls/config_linux')
end
...
local function get_bundles()
    local jar_patterns = {
        vim.fn.glob(
            env.HOME
            -- ~/.vscode-insiders/extensions/vscjava.vscode-java-debug-0.49.2023032407
            .. "/.vscode-insiders/extensions/vscjava.vscode-java-debug-0.49.2023032407/server/com.microsoft.java.debug.plugin-*.jar"
        ),
    }
    vim.list_extend(
        jar_patterns,
        vim.split(
        -- ~/.vscode-insiders/extensions/vscjava.vscode-java-test-0.38.2023032402
            vim.fn.glob(env.HOME .. "/.vscode-insiders/extensions/vscjava.vscode-java-test-0.38.2023032402/server/*.jar"),
            "\n"
        )
    )
    return jar_patterns
...
local function get_jar_patterns()
    local jar_patterns = {}
    local plugin_path = path.join(env.HOME,
        "/.vscode-insiders/extensions/vscjava.vscode-java-test-0.38.2023032402/server")
